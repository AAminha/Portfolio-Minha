...
export const Header = () => {
  ...
  return (
    <header className="fixed w-full bg-shadow-primary">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-14 py-4">
        {/* TODO: 로고 클릭 시 스크롤 최상단으로 이동 */}
        <LogoWhite className="cursor-pointer" />
        <nav className="">
          <ul className="header-4 xs:hidden flex gap-x-10 text-white sm:hidden md:hidden lg:hidden">
          ...
          <MenuIcon
            className="3xl:hidden cursor-pointer xl:hidden 2xl:hidden"
        ...
        <nav className="mx-14 py-2">
          <ul className="header-4 text-white">
            ...
            {NavContent.map((content) => (
              <li
                key={content.title}
                className="cursor-pointer px-12 py-6 hover:text-secondary-300"
              >
                {content.title}
              </li>
            ))}
